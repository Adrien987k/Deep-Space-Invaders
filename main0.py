...
class StackProcessor(object):
    ...
    def stack_frame(self, frame, new_episode):
        ...
        return torch.tensor(np.stack(self.frame_stack, axis=2), dtype = torch.float)
...
#policy_net = DQN(StackProcessor.screen_height, StackProcessor.screen_width).to(device)
#target_net = DQN(StackProcessor.screen_height, StackProcessor.screen_width).to(device)
policy_net = DQN(4, N).to(device)
target_net = DQN(4, N).to(device)
...
def select_action(policy_net, state):
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            ...
            return torch.eye(N)[a.max(1)[1].view(1, 1)[0][0]]
    else:
        return torch.eye(N)[random.randrange(N)]
...
def optimize_model(policy_net, memory):
    ...
    state_action_values = state_action_values.gather(1, action_batch.type(torch.LongTensor))
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        action = select_action(policy_net, state).type(torch.FloatTensor)
        print(action)
